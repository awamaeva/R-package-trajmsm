...
sub_pltmle2<-function(formula,Y,A,L,V,s,time,timevar,idvar,obsdata,Traj){
  A. = paste0(A,".")
  L. = paste0(L,".")
  Y. = paste0(Y,".")
  ...
  W = IPW(numerator = "unstabilized", id = idvar , V = V,L =L.,A = A., C = FALSE,
            censor = censor, K = s,obsdata = obsdata);
  ...
  modEs = glm(as.formula(paste0(paste0(Y.,s), "~", "-1 + offset(logit(unlist(Qs))) + Hs")), family = binomial, data = obsdata.all2);
  coef_Es <- coef(modEs)
  ...
  Ds = lapply(1:nb_regimes,function(x)sapply(1:J,function(y)
    (Hs.all[[x]][,y]*(obsdata[,paste0(Y.,s)] - Qstar[[x]]))))
